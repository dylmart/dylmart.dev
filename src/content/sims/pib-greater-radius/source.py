...
ring2 = ring(pos = vec(15,0,0),
             ...
             thickness = radius,
             color = color.cyan)             
line1 = cylinder(pos = vec(-15,-7.5,0),
                 axis = vec(30,0,0),
                 radius = radius,
                 color = color.cyan)
line2 = cylinder(pos = vec(-15,7.5,0),
                 axis = vec(30,0,0),
                 radius = radius,
                 color = color.cyan)   
line3 = cylinder(pos = vec(-15,0,7.5),
                 axis = vec(30,0,0),
                 radius = radius,
                 color = color.cyan)
line3 = cylinder(pos = vec(-15,0,-7.5),
...
for theta in arange(0, theta_total, d_theta):
    ...
    segment = cylinder(pos = vec(coil_left, coil_radius*cos(theta), coil_radius*sin(theta)),
                       axis = coil_radius*d_theta*vec(0,-sin(theta+d_theta),cos(theta+d_theta)),
                       radius = 0.5,
                       color = color.orange,
                       opacity = 0.35)
...
while True:
    ...
    for POI in POIs:
        if POI.inside == True:
            ...
            if 7.5 < mag(vec(0,POI.pos.y,POI.pos.z)) or POI.pos.x > 15 or POI.pos.x < -15:
